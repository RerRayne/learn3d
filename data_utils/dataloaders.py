...
class UnknownDataTypeError(Exception):
    def __init__(self, *args):
        if args:
            ...
            self.message = 'Datatype not understood for dataset.'
...
class RegistrationData(Dataset):
    def __init__(self, algorithm, data_class=ModelNet40Data(), partial_source=False, partial_template=False,
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'use_rri'):
            if self.use_rri:
                template, source = template.numpy(), source.numpy()
                template = np.concatenate(
                    [template, self.get_rri(template - template.mean(axis=0), self.nearest_neighbors)], axis=1)
                source = np.concatenate([source, self.get_rri(source - source.mean(axis=0), self.nearest_neighbors)],
                                        axis=1)
